...
SELECT DATENAME(MONTH,start), CAST(ROUND(AVG(total_claim_cost),-2) AS INT) AS AverageClaimCost
FROM encounters
GROUP BY DATENAME(MONTH,start), MONTH(start)
ORDER BY MONTH(start)

--Total Claim Cost by Health Reason - Top 10
SELECT TOP 10 reasondescription, SUM(total_claim_cost) AS TotalClaimCost
FROM Encounters
...
WITH CostRange AS (SELECT id, total_claim_cost,
	CASE
...
SELECT range, COUNT(total_claim_cost) AS encounters
FROM CostRange
GROUP BY range
ORDER BY encounters DESC

--Total Claim Cost over Time
SELECT YEAR(start) AS year, SUM(total_claim_cost)
FROM encounters
GROUP BY YEAR(start)
ORDER BY year

--Insurance Coverage Proportion by Race
SELECT p.race, SUM(e.total_claim_cost) AS TotalClaimCost, SUM(e.payer_coverage) AS PayerCoverage,
CASE 
...
SELECT p2.name, SUM(e.total_claim_cost) AS TotalClaimCost, SUM(e.payer_coverage) AS PayerCoverage,
CASE
	WHEN SUM(e.total_claim_cost) > 0 THEN FORMAT(SUM(e.payer_coverage) / SUM(e.total_claim_cost)*100, 'N2')
	ELSE '0.00'
