--Total revenue
SELECT FORMAT(SUM(total_claim_cost),'N2') AS Total
FROM encounters
...
--Readmitted Patients
WITH admission_count AS (SELECT patient, COUNT(*) AS admissions
FROM encounters
GROUP BY patient
HAVING COUNT(*) > 1)
...
WITH admission_count AS (
    SELECT patient, COUNT(*) AS admissions
    FROM encounters
    GROUP BY patient
    HAVING COUNT(*) > 1
)
SELECT 
CAST(
	(CAST((SELECT COUNT(*) FROM admission_count) AS DECIMAL(10, 2)) /
     CAST((SELECT COUNT(DISTINCT patient) FROM encounters) AS DECIMAL(10, 2))
    ) * 100 
AS DECIMAL(10, 2)) AS ReadmissionRate
...
SELECT p.city, p.county, p.state, COUNT(*) AS TotalEncounters
FROM encounters e
